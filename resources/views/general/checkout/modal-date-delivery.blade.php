<div class="modal rounded-xl w-max bg-white" data-trigger-modal="{{ $section }}">
    <section class="flex items-center justify-center border-b border-light-gray-100 p-3">
        ...
        <div class="font-bold">{{ $supplierName }} - Pilih Waktu</div>
    </section>
    ...
    <section class="p-4 flex gap-4 border-b border-light-gray-100 ">
        <x-button class="py-2 px-4 text-sm" data-supplier="{{ $supplierSlug }}" data-section-target="date-today" buttonStyle="secondary" value="Hari ini, {{ str_replace('2024', '', $today) }}"/>
        <x-button class="py-2 px-4 text-sm" data-supplier="{{ $supplierSlug }}" data-section-target="date-tomorrow" buttonStyle="outline-secondary" value="Besok, {{ str_replace('2024', '', $tomorrow) }}"/>
        <x-button class="py-2 px-4 text-sm" data-supplier="{{ $supplierSlug }}" data-section-target="date-after-tomorrow" buttonStyle="outline-secondary" value="{{ $dayAfterTomorrow }}, {{ str_replace('2024', '', $afterTomorrow) }}"/>
    </section>

    <section class="p-4 grid grid-cols-2 gap-4" data-supplier="{{ $supplierSlug }}" data-delivery-price="{{ $deliveryPrice }}" data-section="date-today">
    ...
    <section class="p-4 hidden grid-cols-2 gap-4" data-supplier="{{ $supplierSlug }}" data-delivery-price="{{ $deliveryPrice }}" data-section="date-tomorrow">
    ...
    <section class="p-4 hidden grid-cols-2 gap-4" data-supplier="{{ $supplierSlug }}" data-delivery-price="{{ $deliveryPrice }}" data-section="date-after-tomorrow">
