<div @class([
    ...
    <form action="{{ route('home.search', ['key' => $search_key]) }}" @class([
        ...
        'after:absolute after:z-10 after:bottom-2 after:ml-4 after:h-[1px] after:w-[94.5%] after:bg-light-gray-100' => ! $isAllEmpty,
        'shadow-input rounded-t-md rounded-x-md border-secondary !border-b-white'                                   => ! $isAllEmpty,
        'rounded-md border-white'                                                                                   => $isAllEmpty,
    ])>
        <input type="search" class="py-1.5 px-4 w-full text-sm" name="search_key" placeholder="Mau beli apa hari ini?" wire:model.live.debounce.250ms="search_key">
        <button type="submit" class="mr-4 rounded py-1 px-3 bg-tertiary">
            ...
            <div class="swiper" data-swiper-id="banner-search">
                ...
                @foreach ($result['banners'] as $banner)
                    ...
                    <div class="swiper-slide">
                        <a href="{{ $banner['banner_redirect_url'] ?? route('page.banner', ['banner' => $banner['banner_slug']]) }}" data-banner-name="{{ $banner['banner_name'] }}" >
    ...
    @unless (empty($result['keywords']))
        ...
        <section data-section="keywords">
            ...
            @foreach ($result['keywords'] as $keyword)
                ...
                <li>
                    <x-nav-link href="{{ route('home.search', ['key' => $search_key]) }}" class="py-1 px-4 hover:bg-secondary-50" :value="$keyword['keyword_name']"/>
                </li>
    ...
    @unless (empty($result['categories']))
        ...
        <section data-section="categories">
            ...
            @foreach ($result['categories'] as $keyword)
                ...
                <li>
                    <a href="{{ route('home.search', ['key' => $search_key, 'category' => $keyword['category_name']]) }}" class="block py-1 px-4 hover:bg-secondary-50">
                        <div>{{ $keyword['keyword_name'] }}</div>
                        <h4>di Kategori <span class="text-primary-600">{{ $keyword['category_name'] }}</span></div>
                    </a>
                </li>
...
@push('scripts')
<script>
    const searchInput = document.querySelector('[name="search_key"]');
