<header class="mb-6">
    ...
    <section>
        ...
        @php $isCreatePage = isRouteContains('create') @endphp

        <ol class="inline-flex items-center text-sm">
            <li class="text-light-gray-400">
                <x-nav-link class="leading-none cursor-pointer hover:text-secondary" href="{{ route('products.index') }}" value="Produk"/>
            </li>
            ...
            <li class="flex items-center">
                <x-icon class="mx-2 h-3 grayscale" src="{{ asset('img/icons/icon-header-chevron-right.webp') }}"/>
                <span class="leading-none text-black">{{ slugToTitle($productSlug) }}</span>
            </li>
